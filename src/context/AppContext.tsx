...
export function resolveUsernameAndReferralCode(raw: {
  ...
  const cleanU = (raw.username || '').trim();
  const cleanR = (raw.referralCode || '').trim();

  // If username exists and is not just an old autogenerated GOLD... code (e.g. GOLDABCD), keep it!
  const isOldGoldCodeGen = /^GOLD[A-Z0-9]{4,8}$/i.test(cleanU);
  if (cleanU && !isOldGoldCodeGen) {
    finalUser = cleanU;
  } else if (cleanR && !/^GOLD[A-Z0-9]{4,8}$/i.test(cleanR)) {
    finalUser = cleanR;
  } else if (cleanU) {
    finalUser = cleanU;
  } else if (emailPrefix) {
  ...
  const loginUser = async (userData: { 
    ...
    try {
      ...
      const freshUserState: UserState = {
        ...
        referralEarnings: remoteProfile?.referralEarnings !== undefined ? Number(remoteProfile.referralEarnings) : (userState.referralEarnings || 0.00),
        l1Referrals: remoteProfile?.l1Referrals !== undefined ? Number(remoteProfile.l1Referrals) : (userState.l1Referrals || 0),
        l2Referrals: remoteProfile?.l2Referrals !== undefined ? Number(remoteProfile.l2Referrals) : (userState.l2Referrals || 0),
        l3Referrals: remoteProfile?.l3Referrals !== undefined ? Number(remoteProfile.l3Referrals) : (userState.l3Referrals || 0),
        l1Earnings: remoteProfile?.l1Earnings !== undefined ? Number(remoteProfile.l1Earnings) : (userState.l1Earnings || 0.00),
        l2Earnings: remoteProfile?.l2Earnings !== undefined ? Number(remoteProfile.l2Earnings) : (userState.l2Earnings || 0.00),
        l3Earnings: remoteProfile?.l3Earnings !== undefined ? Number(remoteProfile.l3Earnings) : (userState.l3Earnings || 0.00),
        teamSize: remoteProfile?.teamSize !== undefined ? Number(remoteProfile.teamSize) : (userState.teamSize || 0),
        teamRecharge: remoteProfile?.teamRecharge !== undefined ? Number(remoteProfile.teamRecharge) : (userState.teamRecharge || 0.00),
      };

      setUserState(freshUserState);

      // Instantly trigger live team reconciliation for the logged-in user
      syncAndRefreshUserTeam(userData.uid, {
        username: effUser,
        referralCode: effCode
      }).catch(() => {});
